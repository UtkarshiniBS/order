    ...
    const WalmartScript = async function () {
        ...
        class OrderDetailsScraper extends Scraper {
            constructor() {
                ...
                this.selectors = {
                    ...
                    PaymentSectionSelector: '.bill-order-payment-divider',
                    ...
                    itemSellerSelector1: '[data-testid="seller-name-link"]',
                    ...
                    paymentMethodSelector:
                        '.bill-order-payment-card span, .bill-order-payment-card img',
                    purchaseTypeSelector: 'span[id^="caption-"]'
                };
            }
            ...
            async scrape() {
                ...
                let paymentMethod = [];
                // Remove payment Span if img tag is also available
                paymentObject = paymentObject.filter((tagValue, tagIndex) => {
                    if (
                        !(
                            paymentObject[tagIndex - 1] &&
                            paymentObject[tagIndex - 1].tagName === 'IMG'
                        ) &&
                        tagValue
                    ) {
                        return true;
                ...
                paymentObject.forEach(g => {
                    if (g.tagName === 'IMG') {
                        paymentMethod.push(g.alt);
                    } else {
                        const value = g.innerText.trim().replace(/[0-9]/g, '');
                        if (value) {
                ...
                orderCardAll.forEach(card => {
                    const ptype = card.querySelector(this.selectors.purchaseTypeSelector);
                    const purchaseType = ptype ? ptype.textContent : card.innerText.split('\n')[0];
                    const itemStatusDate = card
                        .querySelectorAll('h2')[0]
                        .textContent.trim();
                    let cardItems = Array.from(
                        card.querySelectorAll(this.selectors.itemCardSelector),
                    );
                    const seller = card.querySelector(this.selectors.itemSellerSelector1);
                    for (let cardItem of cardItems) {
                        const itemCard = Array.from(
                            cardItem.querySelectorAll(this.selectors.orderCardChildren),
                        );
                        for (let item of itemCard) {
                            const itemData = {};
                            const anchor = item.querySelector(
                                this.selectors.itemLinkSelector,
                            );
                            ...
                            // }
                            if (seller) {
                                itemData['itemSeller'] = seller.textContent.trim();
                            }
                            itemData['itemName'] = nextEl
                                .querySelector(this.selectors.itemNameSelector)
                                .textContent.trim();
                            const itemAttributes = Array.from(
                                nextEl.querySelectorAll(this.selectors.itemAttributeSelector),
                            ).filter(el => {
                                if (!el.querySelector(this.selectors.itemUrlSelector)) {
                                    return true;
                                } else {
                                    itemData['itemUrl'] = el.querySelector(
                            ...
                            itemData['itemPrice'] = nextEl
                                .querySelector(this.selectors.itemPriceSelector)
                                .textContent.trim();
                            if (nextEl.querySelector(this.selectors.itemUnitPriceSelector)) {
                                itemData['itemUnitPrice'] = nextEl
                                    .querySelector(this.selectors.itemUnitPriceSelector)
                                    .textContent.trim();
                            }
            ...
            getDOMelement() {
                ...
                const orderSumary = Array.from(
                    orderSummaryContainer.firstElementChild.children,
                ).find(d => d.textContent === 'Payment method');
                // let paymentObject = Array.from(orderSumary && orderSumary.nextSibling ? orderSumary.nextSibling.lastElementChild.children : []);
                let paymentObject = Array.from(
                    document.querySelectorAll(this.selectors.paymentMethodSelector),
                );
        ...
        const onInit = async function () {
            ...
            if (configData.type === LISTING_WORKFLOW) {
                ...
                await delay(500);
                const listingUrl = configData.urls.listing.trim().toLowerCase();
