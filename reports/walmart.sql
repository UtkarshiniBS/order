...
select count(1) from "walmartConnection" where created_at between '2021-10-29 00:00:00' and '2021-11-25 00:00:00' and panelist_id in (select uuid from ios_468_users);

-- Connections Breakdown - IOS users
select count(1), status, order_status,message from "walmartConnection" ac where created_at between '2021-10-29 00:00:00' and '2021-11-25 00:00:00'  
...
select distinct message, count(*) from (select  created, message, count(*)  from (
	select distinct al.panelist_id ,al.platform_id ,al.status ,cast(al.created_at as DATE) as created, al.message from "walmartLogs" al 
	left join "walmartConnection" ac on ac.panelist_id = al.panelist_id and ac.platform_id = al.platform_id
...
from (
        select CAST(created_at as DATE),
            ...
            scrapping_type
        from "walmartOrderHistory"
        where panelist_id in (
                select panelist_id
                from "walmartConnection"
...
oh.id,oh.storage_type, "scrappingType" from "walmartLogs" al left join "walmartOrderHistory" oh on 
...
and al.panelist_id in (select panelist_id from "walmartConnection" where created_at > '2021-10-29 00:00:00') and al.panelist_id in ( select uuid from ios_468_users iul )
...
select distinct al.panelist_id ,al.platform_id ,al.status,al.from_date ,al.to_date  ,cast(al.created_at as DATE) as created,
oh.id,oh.storage_type, "scrappingType" ,message,type from "walmartLogs" al left join "walmartOrderHistory" oh on 
oh.panelist_id =al.panelist_id and oh.platform_id =al.platform_id and oh.from_date =al.from_date and oh.to_date =al.to_date
where oh.id is null and al.status ='fail' and al."section" ='orderUpload'
and al.panelist_id in (select panelist_id from "walmartConnection" where created_at > '2021-10-29 00:00:00' and panelist_id in ( select uuid from ios_468_users iul ) )
and al.created_at between '2021-10-29 00:00:00' and '2021-11-25 00:00:00'
...
select count(1) from "walmartConnection" where created_at between '2021-11-24 00:00:00' and '2021-11-25 00:00:00' and panelist_id in (select uuid from android_234_users);

-- Connections Breakdown - Android users
select count(1), status, order_status,message from "walmartConnection" ac where created_at between '2021-11-24 00:00:00' and '2021-11-25 00:00:00'
...
oh.panelist_id =al.panelist_id and oh.platform_id =al.platform_id and oh.from_date =al.from_date and oh.to_date =al.to_date
where oh.id is null and al.status ='fail' and al."section" ='orderUpload' 
and al.panelist_id in (select panelist_id from "walmartConnection" where created_at > '2021-11-24 00:00:00') and al.panelist_id in ( select uuid from android_234_users au )
...
oh.panelist_id =al.panelist_id and oh.platform_id =al.platform_id and oh.from_date =al.from_date and oh.to_date =al.to_date
where oh.id is null and al.status ='fail' and al."section" ='orderUpload'
and al.panelist_id in (select panelist_id from "walmartConnection" where created_at > '2021-11-24 00:00:00' and panelist_id in ( select uuid from android_234_users iul ) )
and al.created_at between '2021-11-24 00:00:00' and '2021-11-25 00:00:00'
order by al.panelist_id,al.from_date ,al.to_date ) a group by a."scrappingType",message,type order by count(*) desc;
